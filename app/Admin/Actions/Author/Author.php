...
use App\Models\Admin\Author as AuthorModel;
use Illuminate\Http\Request;
use Encore\Admin\Actions\RowAction;
use Illuminate\Database\Eloquent\Model;

class Author extends RowAction
{
    public $name = "作者信息";
    ...
    public function handle(Model $model, Request $request)
    {
        ...
        if (!empty($uid) && !empty($isForbidden)) {
            if (AuthorModel::query()->where("uid", "=", $uid)->update(["is_forbidden" => $isForbidden])) {
    ...
    {
        $authorUid = $this->getRow()->getAttribute("uid");
        $author = AuthorModel::query()->where("uid", "=", $authorUid)->first(["series_count", "created_at", "is_forbidden", "uid"]);
        ...
        $this->radio('is_forbidden', '账号状态')->default($author->is_forbidden)->options([1 => "禁用", 2 => "正常"]);
    }
}
